...
def ExtNovelAPA(outDir, fkey, ref_bed, ref_fasta, md, anchor, iplen, novelD, prpA, npc, mode, samples,logfile, strand):	
	...
	for line in lines:
		...
		if data[5] == '+' and data[0] != 'chrM':
			if ((int(data[2]) - 30) > 0):
				fw.write(data[0] + '\t' + str(int(data[2]) - 30) + '\t' + str(int(data[2]) + iplen) + '\t' + data[0] + '_' + data[1] + '_' + data[2] + '_' + data[3].replace('_', '-') + '_' + data[4].replace('_', '-') + '_' + data[5] + '\tN\t' + data[5] + '\n')
			else:
		...
		if data[5] == '-' and data[0] != 'chrM':
			if ((int(data[1]) - iplen) > 0):
				fw.write(data[0] + '\t' + str(int(data[1]) - iplen) + '\t' + str(int(data[1]) + 30) + '\t' + data[0] + '_' + data[1] + '_' + data[2] + '_' + data[3].replace('_', '-') + '_' + data[4].replace('_', '-') + '_' + data[5] + '\tN\t' + data[5] + '\n')
			else:
				fw.write(data[0] + '\t' + str(int(data[1])) + '\t' + str(int(data[1]) + 30) + '\t' + data[0] + '_' + data[1] + '_' + data[2] + '_' + data[3].replace('_', '-') + '_' + data[4].replace('_', '-') + '_' + data[5] + '\tN\t' + data[5] + '\n')
